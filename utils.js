...
const TYPE_COLOR = { add: 0x00C853, deduct: 0xD32F2F, set: 0xFFA000, reset: 0x607D8B };
...
async function sendLog(guild, settings, data, forcedChannelId = null) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '📊 التغيير',         value: `\`${TYPE_SIGN[data.type] ?? ''}${data.points}\``,  inline: true  },
      { name: '🏆 المجموع الجديد',  value: `\`${data.newTotal}\``,                             inline: true  },
      { name: '📝 السبب',           value: data.reason || '—',                                 inline: false },
      { name: '👮 بواسطة',          value: `${data.executorTag} (<@${data.executorId}>)`,      inline: false },
    )
    .setTimestamp();
...
async function buildReportEmbed(guild, settings, since, periodLabel) {
  ...
  const lines = await Promise.all(leaderboard.map(async (row, i) => {
    let name;
    try { name = (await guild.members.fetch(row.user_id)).user.username; }
    catch { name = `<@${row.user_id}>`; }
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: '➕ عمليات الإضافة', value: `\`${stats.add_count}\` (${stats.total_added} نقطة)`,    inline: true  },
      { name: '➖ عمليات الخصم',   value: `\`${stats.deduct_count}\` (${stats.total_deducted} نقطة)`, inline: true },
      { name: '👥 أعضاء نشطون',    value: `\`${stats.unique_members}\``,                             inline: true  },
      { name: '🏆 أفضل 5 أعضاء',  value: lines.length ? lines.join('\n') : 'لا توجد بيانات',        inline: false },
    )
    .setTimestamp();
}

module.exports = { canAddPoints, isOwner, sendLog, checkPromotion, buildReportEmbed };
